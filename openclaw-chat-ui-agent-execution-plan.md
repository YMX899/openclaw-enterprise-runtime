...
- [x] M1 阶段A：ChatGPT 风格 UI + 聊天内视频输入
      （commits b3f5373 + 03bcf48，已部署 root release video-agent-fix-20260608T0001，
      真实账号登录验收通过：登录弹窗、登录后聊天界面、composer 链接检测、零页面错误、
      零横向溢出；证据 artifacts/evidence/phase4/openclaw-chatgpt-ui-root-evidence-20260608.json）
- [ ] M2 阶段B：Agent 身份/意图/分支/错误处理
- [ ] M3 阶段C：skill 固化
- [ ] M4 阶段D：端到端测试与最终证据

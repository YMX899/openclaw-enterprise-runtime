...
- Users log in on the OpenClaw page with an OpenClaw-owned account/password
  form.
- Bridge validates submitted account/password server-side against the Huahuo
  frontend user system. It must fail closed when Huahuo login rejects the
  credentials or the Huahuo user-info lookup cannot produce a stable user
  identity.
...
- Dify `accounts` rows, Dify Web cookies, Dify Web headers and browser login
  state are not OpenClaw login gates.

...
|---|---|---|
| Use git version management | Partial complete | Git repo initialized; latest committed state is `1d0f1e3` with tag `douyin-real-sample-runner-20260606`. Multiple rollback tags exist. Server-side versioned deployment still pending. |
...
| Docker compose sidecar | Partial | Compose draft exists and static YAML check passed; Docker CLI unavailable locally; not rendered/built/deployed. Phase 1.5 gate now has `REQUIRE_OPENCLAW_SECURITY_APPROVAL=1` for exit, which currently fails by design. |
